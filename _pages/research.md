...
Prior work by *Vaze and Sinha* established that both regret and total constraint violation can be bounded by \( O(\sqrt{T}) \), a result that was widely believed to be optimal. In this work, we refute that belief and show that, in two dimensions, it is possible to achieve:
- Regret: \( O(\sqrt{T}) \)  
